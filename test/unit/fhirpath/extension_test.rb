...
class ExtensionTest < Test::Unit::TestCase
  ...
  def test_extension
    result = FHIRPath.evaluate('Patient.extension', PATIENT)
    assert result == EXT_ARRAY, 'Failed to resolve extension.'
  end

  def test_extension_with_block
    result = FHIRPath.evaluate("Patient.extension('http://hl7.org/fhir/StructureDefinition/us-core-race')", PATIENT)
    assert result == EXT, 'Failed to resolve extension by name.'
  end

  def test_extension_item
    result = FHIRPath.evaluate('Patient.extension[0]', PATIENT)
    assert result == EXT, 'Failed to resolve extension by index.'
  end

  def test_extension_value
    result = FHIRPath.evaluate('Patient.extension[0].value', PATIENT)
    assert result == CONCEPT, 'Failed to resolve extension value.'
  end

  def test_extension_with_block_value
    result = FHIRPath.evaluate("Patient.extension('http://hl7.org/fhir/StructureDefinition/us-core-race').value", PATIENT)
    assert result == CONCEPT, 'Failed to resolve named extension value.'
  end

  def test_extension_missing
    result = FHIRPath.evaluate("Patient.extension('http://hl7.org/fhir/StructureDefinition/us-core-ethnicity')", PATIENT)
